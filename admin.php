   ...
   &nbsp;&nbsp;<SELECT name="job" onChange="doForm.submit()">
    <OPTION SELECTED value="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &lt;Select an Option&gt; </OPTION>
     ...
     <OPTGROUP label="Sharing Tree Admin">
      ...
      <OPTION value="adduser">Add Agency User</OPTION>
      <OPTION value="resetpwd">Reset My Password</OPTION>
      
       <!-- 
      <OPTION value="fixtags">Fix Agency Tags</OPTION>
...
<?
      ...
      //=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
		...
		//=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      {  
         $clQuery="Select AgTag, Tag_ID, First_Name, Last_Name, Middle_Name, Suffix_Name, DOB, Phone_Number, Gift_Description, Price, Tag, Category, agname from AllTagInfo ORDER BY Tag_ID ASC";
         $clResult = mysqli_query($conn, $clQuery) or die ("ERROR AD1298:  SEARCH ERROR  ".mysqli_error($conn));
...
<?
      ...
      {
?>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" name="nuserForm">
<input type="hidden" name="pass2" value="newaguser">
<table class="editor">
<tr class="editor">
 <td class="editor"><div class="ed2">Username:</td></div>
 <td class="editor"><div class="ed3"><input type="text" name="rauser" id="ruser" required/></div></td>
</tr>
<tr class="editor">
 <td class="editor"><div class="ed2">Agency:</td></div>
 <td class="editor"><div class="ed3"><input type="text" name="raagency" id="ragency" onchange="getAgencyCode()" /></div></td>
</tr>
<tr class="editor">
  <td class="editor"><div class="ed2">Agency Code:</td></div>
 <td class="editor"><div class="ed3"><input type="text" name="raagcode" id="ragcode" readonly required /></div></td>
</tr>
<tr class="editor">
 <td class="editor"><div class="ed2">Permissions:</td></div>
 <td class="editor"><div class="ed3">
 <SELECT name="acctype" required>
  <OPTION value="admin">Admin</OPTION>
  <OPTION value="agencyadm">Agency</OPTION>
...
<p><span id="pwdstatus"></span></p>
<button type="button" name="Submit Form" class="btn" onclick="submitNewUser()">Create User</button>
</form>
<?      
         break;   
      }
      case "fixtags";
      {
         print "Fixing agency tags.....";
         
         $agQ = "SELECT DISTINCT accid FROM Tag";
         $agR = mysqli_query($conn,$agQ) or die ("ERROR AD1496:  SEARCH ERROR  ".mysqli_error($conn));
         ...
         while($agRow = mysqli_fetch_assoc($agR))
         {
            ...
            print $colQ."<br>";
            $colR = mysqli_query($conn,$colQ) or die ("ERROR AD1503:  SEARCH ERROR  ".mysqli_error($conn));
            
            while($colRow = mysqli_fetch_assoc($colR))
            {
               $updQ = "SELECT Tag_ID FROM Tag WHERE accid='".$agRow['accid']."' AND Tag='".$colRow['Tag']."'";
               print $updQ."<br>";
               $updR = mysqli_query($conn,$updQ) or die ("ERROR AD1509:  SEARCH ERROR  ".mysqli_error($conn));
               $count = 1;
               
               while($updRow = mysqli_fetch_assoc($updR))
               {
                  //Number them up by color and agency
                  $cntQ = "UPDATE Tag SET agtagnum='".$count."' WHERE Tag_ID='".$updRow['Tag_ID']."'";
                  print $cntQ."<br>";
                  $cntR = mysqli_query($conn,$cntQ) or die ("ERROR AD1517:  SEARCH ERROR  ".mysqli_error($conn));
                  $count ++;
               }
            }
         }         
...
elseif (array_key_exists('pass2', $_POST))
{
   ...
   switch($pass2)
   {
      case "submittags";
      {
         ...
         if($tag == "Green")
         {
            //print "DOBM<BR>=$dobm=<br>DOBD<BR>=$dobd=<br>DOBY<BR>=$doby=<br>";
            ...
            if(mysqli_num_rows($agR) == 0) { $agnum = 0; }
            $agnum ++;
      ...
      case "newaguser";
      {
         $username = htmlentities($_POST['rauser']);
         $password = htmlentities($_POST['rpassword']);
         $agname = htmlentities($_POST['raagency']);
         $acctype = htmlentities($_POST['acctype']);
         $salt = randomString(2);
         $cryptpw = crypt($password, $salt);
         print "acctype:  $acctype<br>";
         
         $accQ = "INSERT INTO Account (userid,password,agcode) VALUES ('$username','$cryptpw','$agname')";
         //print "q1:  $accQ <br>";
         insertData($accQ);
                  
         //Get accid from Accounts
         $idQ = "SELECT accid from Account WHERE userid='".$username."'";
         $idR = mysqli_query($conn, $idQ) or die ("ERROR AD1568: DATA ERROR  ".mysqli_error($conn));
         $idRow = mysqli_fetch_assoc($idR);
         $accid = $idRow['accid'];
         
         //Set perms
         $perQ = "INSERT INTO AccountPerms (accid,$acctype) VALUES ('$accid','1')";
         //print "q2:  $perQ <br>";
         insertData($perQ);
         
         print "<b>User Added:</b><br>Agency:  $agname<br>Username:  $username<br>";
         break;
      }
      case "resetpwd";
      {         
